...
export const DEFAULT_TERRAIN_PARAMETERS: TerrainParameters = {
  seed: 'CoMutiNy',
  baseHeight: 52,
  features: {
    mountains: {
...
const sanitizeFeature = <T extends TerrainFeatureSettings>(
  defaults: T,
  override?: Partial<T>,
): T => ({
  ...defaults,
...
export const normalizeTerrainParameters = (
  overrides?: Partial<TerrainParameters> | TerrainParameters,
): TerrainParameters => {
  if (!overrides) {
    return JSON.parse(JSON.stringify(DEFAULT_TERRAIN_PARAMETERS)) as TerrainParameters;
  }
